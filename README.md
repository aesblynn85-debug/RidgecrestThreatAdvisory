...
- **Perplexity Sonar API** — grounded web search for the Live OSINT tab
- **Vercel** — hosting
...
2. Open **SQL Editor** and run the contents of
   ...
   once. It creates every table, the `case_timeline` view, RLS policies, and
   a starter `org_settings` row.
...
Live OSINT search calls Perplexity's Sonar API server-side.

1. Create a key at [perplexity.ai/settings/api](https://www.perplexity.ai/settings/api).
2. Set it as `PERPLEXITY_API_KEY`. Until this is set, every other page works
...
| `PERPLEXITY_API_KEY` | Live OSINT search (server-only) |
...
- **Webhook push**, for a real dispatch system to call directly:
  ...
  Both paths `upsert` on `external_id`, so re-sending the same record updates
  it instead of duplicating it.

### Live OSINT

Search a lead on the OSINT tab (server-side call to Perplexity Sonar), review
the answer and its sources, then optionally attach it to a case and/or an
entity so it shows up in that case's timeline and that entity's profile.
